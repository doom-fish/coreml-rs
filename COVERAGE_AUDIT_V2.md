...
This audit re-validates the v1 audit against MacOSX26.2.sdk by verifying SDK public surface enumeration and re-checking availability attributes. All 92 top-level Objective-C/C CoreML public symbols (interfaces, protocols, enums, exported constants, and top-level C functions) are present and wrapped. Stringly-typed/raw-JSON access patterns and snapshot-based type exposure count as VERIFIED when the public API surface can still reach the underlying framework symbols.

## Not counted (verified as macOS unavailable in MacOSX26.2.sdk)

| Symbol | Kind | Header | Notes |
| --- | --- | --- | --- |
| MLModelCollection | interface | MLModelCollection.h | Marked with `MODELCOLLECTION_SUNSET` macro which applies `API_UNAVAILABLE(tvos, watchos)` and deprecation period `macos(11.0, 14.4)`. In macOS 26.2 this symbol has passed its deprecation window and is effectively unavailable. |
| MLModelCollectionEntry | interface | MLModelCollectionEntry.h | Same deprecation status as MLModelCollection; `MODELCOLLECTION_SUNSET` applies. |
| MLModelCollectionDidChangeNotification | constant | MLModelCollection.h | Same deprecation status as MLModelCollection class; `MODELCOLLECTION_SUNSET` applies. |
...
| Symbol | Kind | Header | Notes |
| --- | --- | --- | --- |
| _(none)_ | - | - | All 92 audited public macOS CoreML symbols are verified as wrapped in coreml-rs. |

## ⏭️ EXEMPT
| Symbol | Kind | Header | Reason | SDK attribute |
| --- | --- | --- | --- | --- |
| _(none)_ | - | - | No public macOS symbols were excluded from the audit scope. All unavailable symbols (MLModelCollection, MLModelCollectionEntry, MLModelCollectionDidChangeNotification) were filtered out of the public surface count and are documented above in "Not counted". | - |
